...
플러그인에 넣지 않고 따로 쓰는 것이 둘 있습니다. **둘 다 아직 없습니다(진행 예정).** 하나는 평소 답변과 처음 쓰는 글의 문체를 정하는 **output style** 입니다. 어느 것을 권할지는 비교 결과로 정하고 그때 여기에 적습니다. 다른 하나는 글 한 편의 AI 신호 점수를 내고 문턱을 넘으면 알리기만 하는 **AI 신호 검사기** 입니다. 사람 글 487편과 Claude 글 487편으로 만들고, 끝까지 손대지 않은 시험 자료의 사람 글 122편에서 한 편도 잡지 않는지 확인합니다. 만들면 [korean-ai-signals](https://github.com/IsthisLee/korean-ai-signals) 에 두고 설치본에는 넣지 않습니다.
...
| 이 플러그인이 쓰던 작성 스킬과 output style | 평소 답변과 처음 쓰는 글의 문체는 사용자가 고른 output style 에 맡겼습니다. 어떤 것을 권할지는 비교 결과로 정합니다 |
...
`claude plugin list` 에 `korean-kit` 이 `enabled` 로 보이면 끝입니다. 새 버전은 `claude plugin update korean-kit` 으로 받습니다. 권하는 output style 은 이 플러그인에 들어 있지 않고, 어느 것을 권할지는 비교 결과로 정합니다.
