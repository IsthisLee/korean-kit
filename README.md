...
그래서 의미 손실이 없는 도구를 찾기로 했습니다. 늘 켜 두는 한국어 도구 가운데 의미 손실을 막는 데 집중한 [fluent-korean](https://github.com/snflkd/fluent-korean) 을 찾아 지금 쓰고 있습니다. 이것을 플러그인에 실을지는 아래 비교 결과로 정합니다.

다만 그때 고른 방법은 도구 설명을 읽고 직접 써 보는 것이었습니다. 몇몇 도구는 KatFishNet(ACL 2025) 같은 연구를 근거로 달아 두었고, 아무 출처도 적지 않은 도구도 있었습니다. **어느 쪽이든 제 글에서 어느 도구가 뜻을 잃지 않는지는 가릴 수 없었습니다. 뜻 손실을 세는 기준이 없어서 0건인지 아닌지를 판정할 수 없었기 때문입니다.**
...
- **3자 서비스와 통신하지 않습니다.** 설치본의 스킬과 스크립트는 네트워크를 쓰지 않습니다. 확인하는 명령은 [SECURITY.md](SECURITY.md)에 있습니다. 표본 수집과 Claude 글 생성은 저장소의 분석 스크립트만 하며 설치본에는 들어가지 않습니다.
